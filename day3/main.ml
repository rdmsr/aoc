...
let rec find_highest s =
  let rec aux idx highest highidx =
    if idx >= String.length s then (highidx, highest)
      ...
      aux (idx + 1) (max highest digit)
        (if digit > highest then idx else highidx)
  in
  aux 0 (-1) (-1)
...
let rec process_line s n =
  let rec aux idx n acc =
    if n = 0 then acc
      ...
      let highidx, highest =
        find_highest (String.sub s idx (String.length s - idx - n + 1))
      in
...
let () =
  ...
  let result1 =
    List.fold_left (fun acc line -> acc + process_line line 2) 0 lines
  in
  let result2 =
    List.fold_left (fun acc line -> acc + process_line line 12) 0 lines
  in
  Printf.printf "1: %d\n" result1;
  Printf.printf "2: %d\n" result2
