...
type item = Lights of string | Button of int list | Joltage of int list
...
let comma_sep p = sep_by (char ',' *> ws) p

let lights =
  char '[' *> take_till (fun c -> c = ']') <* char ']' >>| fun s -> Lights s

let button =
  char '(' *> ws *> comma_sep integer <* ws <* char ')' >>| fun xs -> Button xs

let joltage =
  char '{' *> ws *> comma_sep integer <* ws <* char '}' >>| fun xs -> Joltage xs

let item = lights <|> button <|> joltage
let items = ws *> many (item <* ws)
...
let light_to_mask s =
  String.fold_left
    (fun acc c ->
      match c with '.' -> acc lsl 1 | '#' -> (acc lsl 1) lor 1 | _ -> acc)
    0 s
...
let rec mask_from_button mask len = function
  | [] -> mask
  | x :: xs ->
      let new_mask = mask lor (1 lsl (len - 1 - x)) in
      mask_from_button new_mask len xs
...
let part1 lines =
  ...
  List.fold_left
    (fun acc line ->
      let light =
        line |> List.find (function Lights _ -> true | _ -> false)
      in
      let len, light_mask =
        match light with
      ...
      let find_presses buttons =
        let rec try_size k =
          let combos = combinations k buttons in
          match
            List.find_opt
              (fun combo -> List.fold_left ( lxor ) 0 combo = light_mask)
              combos
          with
          | Some combo -> List.length combo
          | None ->
              if k > List.length buttons then failwith "No combination found"
              else try_size (k + 1)
        in
        try_size 1
      in
      acc + find_presses all_buttons)
    0 parsed
...
let solve_machine buttons target =
  ...
  for i = 0 to n - 1 do
    ...
    let affecting =
      List.filter_map
        (fun j -> if List.mem i buttons.(j) then Some xs.(j) else None)
        (List.init m (fun j -> j))
    in
  ...
  | Solver.SATISFIABLE -> (
      let model =
        match Optimize.get_model opt with Some a -> a | None -> failwith "wtf"
      in
      match Model.eval model sum true with
...
let part2 lines =
  ...
  List.fold_left
    (fun acc line ->
      let joltage_item =
        line |> List.find (function Joltage _ -> true | _ -> false)
      in

      let joltage =
        match joltage_item with Joltage a -> a | _ -> failwith "wtf"
      in

      let all_buttons =
        List.fold_right
          (fun item acc -> match item with Button b -> b :: acc | _ -> acc)
          line []
      in
      acc + solve_machine (Array.of_list all_buttons) (Array.of_list joltage))
    0 parsed
