...
let part1 grid =
  let rec aux acc x y =
    ...
    else if x >= Array.length grid.(0) then aux acc 0 (y + 1)
    else if has_elem grid x y && count_neighbors grid x y < 4 then
      aux (acc + 1) (x + 1) y
    else aux acc (x + 1) y
  in
  aux 0 0 0
...
let part2 grid =
  let rec aux acc x y removed =
    if y >= Array.length grid then
      if removed = false then acc else aux acc 0 0 false
    else if x >= Array.length grid.(0) then aux acc 0 (y + 1) removed
    else if has_elem grid x y && count_neighbors grid x y < 4 then begin
      grid.(y).(x) <- false;
      aux (acc + 1) (x + 1) y true
    end
    else aux acc (x + 1) y removed
  in
  aux 0 0 0 false
